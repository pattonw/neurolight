...
def train_fusion_pipeline(
    n_iterations,
    setup_config,
    emb_mknet_tensor_names,
    emb_loss_tensor_names,
    fg_mknet_tensor_names,
):
    ...
    foreground_net_checkpoint = setup_config["FOREGROUND_NET_CHECKPOINT"]
    ...
    pipeline = (
        ...
        + BinarizeGt(labels_fg, gt_fg)
        + gp.BalanceLabels(gt_fg, loss_weights)
        + gp.PreCache(cache_size=cache_size, num_workers=num_workers)
        + gp.tensorflow.Predict(
            foreground_net_checkpoint,
            graph=f"{foreground_net_checkpoint}.meta",
            inputs={fg_mknet_tensor_names["raw"]: raw_fused},
            outputs={fg_mknet_tensor_names["fg_pred"]: fg_pred},
        )
        + gp.tensorflow.Train(
            "embedding_net",
            optimizer=create_custom_loss(emb_mknet_tensor_names, setup_config),
            loss=None,
            inputs={
                emb_mknet_tensor_names["loss_weights"]: loss_weights,
                emb_mknet_tensor_names["raw"]: raw_fused,
                emb_mknet_tensor_names["gt_labels"]: labels_fg,
                emb_mknet_tensor_names["fg_pred"]: fg_pred,
            },
            outputs={
                emb_mknet_tensor_names["embedding"]: embedding,
                emb_loss_tensor_names["maxima"]: maxima,
                emb_loss_tensor_names["emst"]: emst,
                emb_loss_tensor_names["edges_u"]: edges_u,
                emb_loss_tensor_names["edges_v"]: edges_v,
                emb_loss_tensor_names["ratio_pos"]: ratio_pos,
                emb_loss_tensor_names["ratio_neg"]: ratio_neg,
                emb_loss_tensor_names["dist"]: dist,
                emb_loss_tensor_names["num_pos_pairs"]: num_pos_pairs,
                emb_loss_tensor_names["num_neg_pairs"]: num_neg_pairs,
            },
            gradients={
                emb_mknet_tensor_names["embedding"]: gradient_embedding,
            },
